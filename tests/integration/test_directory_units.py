...
from mcp_rag.indexer import run_index, DIRECTORY_UNIT_OFFSET, MODULE_UNIT_OFFSET
...
def test_nested_directories_processed_bottom_up(tmp_path):
    ...
    dir_paths = [str(c.file_path or "root") for c in dir_calls]
...
def test_directory_summary_includes_child_file_summaries(tmp_path):
    ...
    dir_calls = [c for c in summarizer.calls
                 if c.unit_type == "directory" and c.file_path is not None
