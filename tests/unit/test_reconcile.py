...
def test_diff_no_incoming_all_deleted():
    existing = [
        _stored(1, "function", "foo", "def foo(): pass", 0),
        _stored(2, "function", "bar", "def bar(): pass", 20),
    ]
...
def test_diff_to_keep_preserves_stored_unit_id():
    content = "def x(): pass"
    existing = [_stored(99, "function", "x", content, 5)]
...
def test_diff_removed_unit_deleted():
    existing = [
        _stored(1, "function", "keep_me", "def keep_me(): pass", 0),
        _stored(2, "function", "removed", "def removed(): pass", 20),
    ]
...
def test_diff_key_unit_type_change():
    ...
    existing = [_stored(1, "function", "foo", content, 0)]
    incoming = [_incoming("method", "foo", content, 0)]
...
def test_diff_key_unit_name_change():
    # Same type and offset, different name → different key
    content = "def foo(): pass"
    existing = [_stored(1, "function", "foo", content, 0)]
...
def test_diff_key_char_offset_change():
    # Same type and name, different offset → different key
    content = "def foo(): pass"
    existing = [_stored(1, "function", "foo", content, 0)]
...
def test_diff_multiple_mixed_changes():
    existing = [
        ...
        _stored(2, "function", "modified", "def modified(): return 0", 30),
...
def test_diff_null_unit_name_as_key():
    content = "SELECT 1"
    existing = [_stored(5, "sql", None, content, 0)]
